...
def rewrite_label_span(label, span, label_set=None, text=None):
    ...
    if label_set and label not in label_set:
        # print ("invalid label: ", label, 'of span: ',span)
    ...
    if span == '':
        # print ("null span:)")
        # exit(0)
        return None, None, 'Invalid-Text-Span'
...
class SpotAsocPredictParser(PredictParser):
    ...
    def decode(self, gold_list, pred_list, text_list=None, raw_list=None
        ...
        for gold, pred, text, raw_data in zip(gold_list, pred_list, text_list,
            ...
            instance['gold_spot'], instance['gold_asoc'], instance['gold_record'], gold_counter = self.get_record_list(
            ...
            instance['pred_spot'], instance['pred_asoc'], instance['pred_record'], pred_counter = self.get_record_list(
        ...
        return well_formed_list, counter + pred_counter
    ...
    def get_record_list(self, sel_tree, text=None):
        ...
        for spot_tree in sel_tree:
            ...
            if isinstance(spot_tree, str) or len(spot_tree) == 0:
                counter.update(['incomplete-tree'])
                continue
            ...
            for asoc_tree in spot_tree:
                ...
                pre_label = asoc_label
                asoc_label, asoc_text, res_string = rewrite_label_span(
                    label=asoc_label,
                    span=asoc_text,
                    label_set=self.role_set,
                    text=text
                )
                # if res_string == 'Invalid-Label':
                    # print ("error label: ", pre_label)
                    # exit(0)
                    #
                    # if asoc_label.strip() != 'None':

                counter.update([res_string + '-asoc'])
                # Drop empty generated span
                if asoc_text is None or asoc_text == null_span:
                    counter.update(['empty-generated-span-asoc'])
                    continue
                # Drop empty generated type
                if asoc_label is None:
                    counter.update(['empty-generated-type-asoc'])
                    continue
                # Drop invalid asoc type
                if self.role_set is not None and asoc_label not in self.role_set:
                    counter.update(['invalid-asoc-type-asoc'])
                    continue
        ...
        return spot_list, asoc_list, record_list, counter
