...
class PLFuncPromptCreator(StructureConverter):
        ...
        for asoc in spot_asoc_list:
            ...
            if asoc['asoc'] != []:
                for as_list in asoc['asoc']:
                    ...
                    temp_prompt = f'\tentity_list.append({{"rel_type": "{tmp_rel}", "ent1_type": "{start_type}", "ent1_text": "{tmp_start}", "ent2_type": "{end_type}", "ent2_text": "{tmp_end}"}})'
    ...
    def output_to_structure(self, input_dict: dict, output_str: str):
        ...
        entities_relation_list = re.findall(r'entity_list.append\((.+?)\)\n', output_str)
